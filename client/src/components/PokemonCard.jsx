...
export default function PokemonCard({ pokemon, selected, slotIndex, onClick, disabled }) {
  ...
  return (
    <button
      ...
      className={[
        "group relative flex flex-col items-center gap-1 p-2.5 rounded-2xl border-4 transition-all duration-200 text-left w-full overflow-hidden bg-[var(--color-bg-card)]",
        "focus:outline-none focus-visible:ring-4 focus-visible:ring-[var(--color-primary)]",
        selected
          ? "border-[var(--color-primary)] shadow-[0_6px_0_var(--color-primary)] transform -translate-y-1 z-10"
          : disabled
            ? "border-[var(--color-border)] bg-[var(--color-bg-panel)] opacity-50 cursor-not-allowed grayscale shadow-[0_4px_0_var(--color-border)]"
            : "border-[var(--color-border)] shadow-[0_4px_0_var(--color-border)] hover:border-[var(--color-primary-light)] hover:shadow-[0_6px_0_var(--color-primary-light)] hover:-translate-y-1 hover:z-10",
      ].join(" ")}
    ...
    >
      ...
      <div className="absolute left-1/2 -top-2 -translate-x-1/2 -translate-y-full w-44 bg-[var(--color-bg-card)] border-4 border-[var(--color-text-primary)] rounded-2xl shadow-[0_6px_0_var(--color-text-primary)] p-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 pointer-events-none hidden sm:block transform group-hover:-translate-y-[calc(100%+12px)]">
      ...
      <div className="relative w-16 h-16 flex items-center justify-center z-10">
        <img
          ...
          className={[
            ...
            selected ? "scale-125 animate-float" : "group-hover:scale-110",
          ].join(" ")}
